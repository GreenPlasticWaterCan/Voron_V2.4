#!/bin/bash
set -euo pipefail
#This is a bash script to automatically update all MCU's. This needs to be changed to each specific printer. It also uses KCONFIG_CONFIG that need to be setup first.
...
python3 ~/katapult/scripts/flashtool.py -i can0 -u c5ee12715040 -r
sleep 1

#Flash the new firmware over CANBus to the toolhead
echo 'Flashing firwmare'
python3 ~/katapult/scripts/flashtool.py -i can0 -u c5ee12715040 -f ~/klipper/out/klipper.bin
sleep 1
